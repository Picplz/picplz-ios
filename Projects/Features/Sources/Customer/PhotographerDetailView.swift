...
public struct PhotographerDetailView: View {
  ...
  public var body: some View {
    NavigationStack(path: $store.scope(state: \.path, action: \.path)) {
            ...
            VStack(spacing: 20) {
              ForEach(store.photographer.reviews) { review in
                PhotographerReviewCard(
                  ...
                  onLike: { } // TODO: Implement like in detail view if needed
                )
              }
              
              Button(action: { store.send(.reviewListButtonTapped) }) {
                HStack(spacing: 4) {
                  Spacer()
      ...
      switch store.case {
      case let .reviewList(reviewListStore):
        PhotographerReviewListView(store: reviewListStore)
      }
    }
  }
}
