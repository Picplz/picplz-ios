...
struct ReviewSortSelectView: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      VStack(spacing: 0) {
        ForEach(PhotographerReviewListFeature.SortOrder.allCases, id: \.self) { order in
          Button(action: { store.send(.selectOrder(order)) }) {
            HStack {
              Text(order.rawValue)
                .typo(.pParagraph)
                .foregroundStyle(store.selectedOrder == order ? .pBlack : .pGrey4)
              Spacer()
...
#Preview {
  PhotographerReviewListView(
    store: Store(initialState: PhotographerReviewListFeature.State(
      photographerId: UUID(),
